...
# New game variables and assets from freakybird.py
...
class Bird(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        #flap on tongue out
        if tongue_out and not self.clicked:
            self.clicked = True
            self.vel = -6  # Lower jump
        if not tongue_out:
            self.clicked = False
        #handle animation
        self.counter += 1
...
# Pipe and Button classes from freakybird.py
...
# Add pipe group, button, and draw_text
